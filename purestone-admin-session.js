(() => {
  ...
  async function headers(extra={}){const t=await token().catch(()=>''),h={...extra};if(t)h.Authorization=`Bearer ${t}`;const legacy=sessionStorage.getItem('purestone-admin-key');if(!t&&legacy)h['x-admin-key']=legacy;return h}
  async function bootstrap(email,password,legacyKey){const r=await fetch(`${URL}/functions/v1/purestone-admin-auth`,{method:'POST',headers:{'Content-Type':'application/json','x-admin-key':legacyKey},body:JSON.stringify({action:'bootstrap',email,password})}),j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'Configurare nereușită');return j}
  ...
  window.PureStoneAdminSession={login,refresh,token,headers,bootstrap,verify,logout:clear,session:read};
})();