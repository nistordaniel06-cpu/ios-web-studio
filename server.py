...
Optimized for instant mobile response, no reverse-DNS delay, safe logging.
"""
import http.server
import socketserver
import os
...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def address_string(self):
        # Avoid reverse DNS lookup on Android (which hangs for 10-30 seconds)
    ...
    def do_GET(self):
        ...
        if clean_path in ('', '/', '/studio', '/learn', '/templates'):
            ...
            self.end_headers()
            return
        elif 'serviceWorker.js' in clean_path:
            self.send_response(204)
            self.end_headers()
            return
    ...
    def log_message(self, format, *args):
        # Safe formatting without IndexError
...
def run_server(port=PORT):
    ...
    print("=" * 64, flush=True)
    print(f" 👉 Localhost (Android Chrome): http://127.0.0.1:{port}", flush=True)
    print(f" 👉 Localhost:                  http://localhost:{port}", flush=True)
    print(f" 📱 De pe telefon / rețea:     http://{local_ip}:{port}", flush=True)
